...
The text above was ported from pstack. These files extend it:

- `ste100.md` is an example-led STE digest with the substitution table.
- `dictionary.json` and `technical-nouns.txt` hold the STE dictionary and the terms of art it must not flag. Add a term of art to `technical-nouns.txt` (skill directory or repo root) instead of exempting text.
- `check.py` is the deterministic first pass. Run `.venv/bin/python check.py <file>` from the skill directory (setup: `pip install spacy`, then `python -m spacy download en_core_web_sm`). It ignores code, tables, blockquotes, and single-line quotations. For canonical text that must not change, wrap it in `<!-- writing: off -->` and `<!-- writing: on -->`; the checker reports every exempt region and flags a file with more than two.

Run `check.py` on every deliverable before it ships, including prose a delegated agent wrote.

Alan's language rules (the unslop skill's local additions) bind every deliverable: no metaphors (write the mechanism instead), no contrast rhetoric (state the positive claim once), no colon connectors or title-plus-subtitle colons, no semicolons in prose. `check.py` detects none of these. Sweep them by hand on every pass.

## Sentence rhythm, measured

Added 2026-09-10 from seven decision records that Codex on gpt-6-astra rewrote without changing their content. The rewrite kept the word count, raised the sentence count by 30 per cent, and cut commas per sentence by 40 per cent. These rules encode what it did. `check.py` enforces the first two.

1. Aim for a mean of 12 to 14 words per sentence in explanation prose. Twenty is the ceiling.
2. A comma followed by so, and, which, or but that joins two clauses becomes a full stop. Keep the comma for a list.
3. Every sentence has a concrete subject. A concept, a language, or a decision never performs an action.
4. A bullet may hold two sentences, the claim and then the mechanism. It never holds a clause chain.
5. Cite a driver, rule, or section as its own short sentence, never as a parenthesis inside a clause.
6. Backticks only for a file, command, or symbol that exists. Never for a concept.
7. "We" for a choice the team made, at most once per paragraph. Never for what the system does.

Worked pairs, before and after:

- "A component can leave for its own repository when its execution model needs it." became "A component can move to its own repository when its execution model requires it."
- "Languages talk only over HTTP and OpenAPI." became "Applications in different languages communicate only through HTTP and OpenAPI."
- "The monorepo holds two frontends, the Go applications core_api and async_worker, the Python application intelligence, and the packages those applications are built from." became "The monorepo contains two frontends, the Go applications core_api and async_worker, and the Python application intelligence. It also contains their packages."
- "Chosen option: X, because product people raise and review a record where they already work (D1, D2, D5)." became "Chosen option: X, because product people can write and review records where they already work. This meets D1, D2, and D5."
- "Runs on-premises, in a private cloud, and in a public cloud, installed by people who do not work at DataTorque." became "Runs on-premises, in a private cloud, and in a public cloud. People outside DataTorque install it."
