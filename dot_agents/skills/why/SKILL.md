---
name: why
description: "Use for 'why does X work this way', 'why we picked Y', design rationale, regressions, postmortems, or data-backed thresholds. Discovers available evidence sources and queries source control plus any relevant MCP-backed systems, then returns a cited read on decisions and tradeoffs. Use how for runtime behavior."
---
...
Historical context spreads across seven evidence categories: source control history, issue or ticket tracking, long-form documents, real-time team chat, infrastructure observability, error or exception tracking, and product analytics warehouses. You cannot predict from the question alone which one holds the answer, so enumerate the evidence sources available at run time, map each to a category, query the available categories in parallel when possible, then synthesize with explicit confidence calibration. Null results from searched categories are first-class evidence about how the decision was made; report them alongside positive findings. The default is coverage, not minimalism.
...
Before starting investigators, inspect the tools and MCP connectors exposed by the current agent runtime.
...
Source control is available through git; use `gh` too when it is installed and authenticated. For the other six, classify any available connector by its name, instructions, tools, and resource descriptions. If a connector could fit more than one category, choose the one matching its primary evidence. Record ambiguous cases in the coverage map. The skill remains useful with git alone; unavailable categories become explicit gaps.
...
Launch all matching investigators together when subagents are available. One investigator per category lets each specialize in one tool's query vocabulary and result shape. If the runtime has no subagents, investigate the categories as separate passes yourself. Keep every investigation read-only while allowing access to its assigned connector.
...
Use one investigation per category that has a matching source. Each owns exactly one tool or connector.
...
Synthesize the results after every investigation finishes. A separate subagent may do this when supported, provided it can read the cited sources. Otherwise synthesize them yourself.
