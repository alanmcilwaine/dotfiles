...
5. **Verify citations by spot-checking.** You can read the codebase and use available read-only connectors to verify citations; do not write files, commit, or modify external state. If you're uncertain a cited item exists or says what's claimed, check it. Don't propagate errors.
