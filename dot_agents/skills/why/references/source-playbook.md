...
The why skill uses one investigation per available evidence category, each reading a single source-specific playbook below. The playbooks are concrete examples for common connectors; adapt their tool calls for a different connector in the same category.
